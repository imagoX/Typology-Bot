...
from aiogram.utils.exceptions import ChatAdminRequired, UserIsAnAdministratorOfTheChat, CantRestrictSelf
...
async def send_welcome(message: types.Message):
    ...
    if message.chat.id != groupId and message.chat.id != testGroupId and message.chat.id != groupId2:
...
async def welcome_send_info(message: types.Message):
    await message.answer("در حال اجرا هستم. برای راهنمایی بیشتر دستور /help را وارد کنید.")
...
async def new_chat_member(message: types.Message):
    ...
    if message.chat.id != groupId and message.chat.id != testGroupId and message.chat.id != groupId2:
        ...
        chat_id = message.chat.id
        await bot.send_message(chat_id=chat_id, text=f"درود "
...
async def left_chat_member(message: types.Message):
    if message.chat.id != groupId and message.chat.id != testGroupId and message.chat.id != groupId2:
        ...
        await bot.send_message(chat_id=chat_id, text=f"بدرود "
...
# ban user
@dp.message_handler(AdminFilter(is_chat_admin=True), IsReplyFilter(is_reply=True), commands=['بن'],
        ...
        try:
            ...
            if replied_user.id == (await bot.me).id:
                await bot.send_message(chat_id=message.chat.id,
                                       text="من نمی‌توانم خودم را بن کنم.")
                return
            ...
            await bot.send_message(chat_id=message.chat.id,
                                   text="من نمی‌توانم یک ادمین را بن کنم.")


#unban user

# unban user
@dp.message_handler(AdminFilter(is_chat_admin=True), IsReplyFilter(is_reply=True), commands=['آنبن'],
                    commands_prefix='!', chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP])
async def unban(message: types.Message):
    if message.chat.id != groupId and message.chat.id != testGroupId and message.chat.id != groupId2:
        ...
        try:
            ...
            if replied_user.id == (await bot.me).id:
                await bot.send_message(chat_id=message.chat.id,
                                       text="نمی توانم خودم را آنبن کنم.")
                return

            # Try to perform the unban
            await bot.unban_chat_member(chat_id=message.chat.id, user_id=replied_user.id)
            await bot.send_message(chat_id=message.chat.id,
                                   text=f"کاربر "
                                        f"[{replied_user.full_name}](tg://user?id={replied_user.id})"
                                        f" توسط ادمین "
            ...
            await bot.send_message(chat_id=message.chat.id,
                                   text="این کاربر بن نیست.")


# mute user in chat
@dp.message_handler(AdminFilter(is_chat_admin=True), IsReplyFilter(is_reply=True), commands=['میوت'],
                    commands_prefix='!', chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP])
async def mute(message: types.Message):
    if message.chat.id != groupId and message.chat.id != testGroupId and message.chat.id != groupId2:
        ...
        try:
            ...
            if replied_user.id == (await bot.me).id:
                await bot.send_message(chat_id=message.chat.id,
                                       text="من نمی‌توانم خودم را میوت کنم.")
                return

            # Try to perform the mute
            now_time = int(time.time())
            await bot.restrict_chat_member(chat_id=message.chat.id, user_id=replied_user.id,
                                           permissions=types.ChatPermissions(can_send_messages=True,
                                                                             can_send_media_messages=True,
                                                                             can_send_other_messages=True),
                                           until_date=now_time + ban_for)
            await bot.send_message(text=f"کاربر "
                                        f"[{replied_user.full_name}](tg://user?id={replied_user.id})"
                                        f" به مدت "
                                        f"{till_date}"
                                        f" میوت شد.",
            ...
            await bot.send_message(chat_id=message.chat.id,
                                   text="من نمی‌توانم یک ادمین را میوت کنم.")


# unmute user in chat
@dp.message_handler(AdminFilter(is_chat_admin=True), IsReplyFilter(is_reply=True), commands_prefix='!',
                    chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP], commands=['آنمیوت'])
        ...
        try:
            ...
            await bot.restrict_chat_member(chat_id=message.chat.id, user_id=replied_user.id,
                                           permissions=types.ChatPermissions(can_send_messages=True,
                                                                             can_send_media_messages=True,
                                                                             can_send_other_messages=True), )
            await bot.send_message(text=f"خب، "
                                        f"[{replied_user.full_name}](tg://user?id={replied_user.id})"
            ...
            await bot.send_message(chat_id=message.chat.id,
                                   text="من نمی‌توانم یک ادمین را آنمیوت کنم.")
        except CantRestrictSelf:
            # Handle the exception when trying to unmute itself
            await bot.send_message(chat_id=message.chat.id,
                                   text="من نمی‌توانم خودم را آنمیوت کنم.")


# pin chat message
@dp.message_handler(AdminFilter(is_chat_admin=True), IsReplyFilter(is_reply=True),
                    chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP], commands=['پین'], commands_prefix='!')
...
# unpin chat message
@dp.message_handler(AdminFilter(is_chat_admin=True), IsReplyFilter(is_reply=True), commands_prefix='!',
                    chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP], commands=['آنپین'])
...
# delete user message
@dp.message_handler(AdminFilter(is_chat_admin=True), IsReplyFilter(is_reply=True), commands_prefix='!',
                    chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP], commands=['حذف'])
...
@dp.message_handler(chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP], commands=['ادمین'],
                    commands_prefix='!/')
...
@dp.message_handler(chat_type=[types.ChatType.SUPERGROUP, types.ChatType.GROUP], commands=['گزارش'],
                    commands_prefix='!/')
async def report_by_user(message: types.Message):
    if message.chat.id != groupId and message.chat.id != testGroupId and message.chat.id != groupId2:
        ...
        if message.reply_to_message is None:
            await message.reply("شما در حال گزارش نامعتبر هستید. لطفاً به یک پیام ریپلای کنید.")
            return
        ...
        for admin in admins_list:
            try:
                ...
                await bot.send_message(text=f"کاربر: {message.from_user.full_name}\n"
...
@dp.message_handler(
    lambda message: message.text.startswith('/chat') and (message.chat.id == groupId or message.chat.id == testGroupId or message.chat.id == groupId2))
async def chat_handler(message: types.Message):
    ...
    if user_message:
        ...
        try:
            ...
            # If there's an error, edit the processing message to indicate failure
